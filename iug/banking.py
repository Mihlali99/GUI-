...
def finish_reg():
    ...
    for name_check in all_accounts:
        if name == name_check:
            notif.config(fg="red",text="Acount already exists")
            return
        else: 
    ...
    for name in all_accounts:
        if name == login_name:
            ...
            if login_password == password:
                ...
                #Labels
                Label(account_dashboard, text="Account Dashboard", font=('Calibri',12)).grid(row=0,stick=N,pady=10)
                Label(account_dashboard, text="Welcome", font=('Calibri',12)).grid(row=1,stick=N,pady=10)
...
def deposit():
    ...
    deposit_notif.grid(row=3,sticky=N,pady=10)
...
def finish_deposit():
    ...
    current_balance_label.config(text="current_balance : R"+str(updated_balance),fg="green")
...
def withdraw():
    ...
    file = open(login_name, "r")
    file_data = file.read()
    user_details = file_data.spilt('/n')
    details_balance = user_details[4]
    ...
    #Button
    Button(withdraw_screen, text="Finish",font=('Calibr',12),command='finish_withdraw').grid(row=3,sticky=W,pady=5)

def finish_withdraw():
    if amount.get() == "":
        deposit_notif.config(text='Amount is required!', fg="red")
        return 
    if float(withdraw_balance_label.get()) <=0:
        deposit_notif.config(text='Negative currency is not accepted', fg='red')
        return

    file = open(login_name, 'r+')
    file_data = file.read()
    details = file_data.spilt('\n')
    current_balance = details[4]
...
def personal_details():
    ...
    #Labels 
    Label(personal_details_screen, 'Personal details', font=('calibri',12)).grid(row=0,sticky=N,pady=10)
    ...
    Label(personal_details_screen, text="Balance :R"+details_balance, font=('calibri',12)).grid(row=4,sticky=N,pady=10)
...
def login():
    ...
    #Button 
    Button(login_name, text="login", command=login_session, width=15,font=('calibri',12)).grid(row=3,sticky=W,pady=5,padx=5)
...
#labels
Label(master, text = "custom banking Alpha", font=('Calibri',14)).grid(row=0,sticky=N,pady=10)
Label(master, text = "the most secure bank", font=('Calibri',12)).grid(row=1,sticky=N,pady=10)
